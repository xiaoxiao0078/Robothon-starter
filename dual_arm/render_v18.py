...
# 获取绿块精确位置（用于红块堆叠）
green_pos = mC.copy()
print(f"  Green block pos: {green_pos}")
...
# ★ 红块叠在绿上 - 修正：z高度 = 绿块桌面高度 + 绿块高度 + 红块半高
green_top_z = 0.75 + 0.04  # 绿块顶部约0.79
P["stk_L"], P["stki_R"] = get_ik(np.array([green_pos[0], green_pos[1], green_top_z + 0.1]), np.array([0.4, -0.4, 0.9]))
P["stk2_L"], P["stki2_R"] = get_ik(np.array([green_pos[0], green_pos[1], green_top_z + 0.05]), np.array([0.4, -0.4, 0.9]))

# ★ 故障恢复：红块差点滑落 → 重新调整 → 精确放置
P["fail_L"], P["fail_R"] = get_ik(np.array([green_pos[0]-0.02, green_pos[1]+0.02, green_top_z + 0.15]), np.array([0.4, -0.4, 0.9]))
P["fix_L"], P["fix_R"] = get_ik(np.array([green_pos[0], green_pos[1], green_top_z + 0.1]), np.array([0.4, -0.4, 0.9]))
...
STEPS = [
    # 去掉静态开场，直接扫描
    S("Scan Workspace",    "scan_L",    "scan_R",    0.04, 0.04, 0.8, cam="overhead"),
    # Phase 1: 蓝块
    S("Reach Blue",        "reach_b_L", "idle_R",    0.04, 0.04, 1.0, cam="close_l"),
    S("Grasp Blue",        "grasp_b_L", "idle2_R",   0.0,  0.04, 0.8, weld=("A","L"), cam="close_l"),
    S("Lift Blue",         "lift_b_L",  "idle3_R",   0.0,  0.04, 1.0, weld=("A","L"), cam="side"),
    # 交接
    S("Handoff Start",     "ho_L",      "ho_R",      0.0,  0.04, 0.8, weld=("A","L"), cam="front"),
    S("Transfer Fail",     "hf_L",      "hf_R",      0.0,  0.04, 0.8, weld=("A","L"), cam="close_r"),
    S("Re-align",          "hr_L",      "hr_R",      0.02, 0.02, 0.8, weld=("A","L"), cam="close_r"),
    S("Transfer OK",       "ho_L",      "ho_R",      0.0,  0.0,  1.0, weld=("A","R"), cam="wide"),
    # 右臂放下蓝
    S("Place Blue",        "rb_L",      "rb_R",      0.0,  0.0,  0.8, weld=("A","R"), cam="close_r"),
    S("Lower Blue",        "rr_L",      "rr_R",      0.0,  0.0,  0.8, weld=("A","R"), cam="close_r"),
    S("Release Blue",      "ro_L",      "ro_R",      0.04, 0.04, 0.6, cam="close_r"),
    # Phase 2: 绿块
    S("Reach Green",       "rg_L",      "rgi_R",     0.04, 0.04, 1.0, cam="close_l"),
    S("Grasp Green",       "gg_L",      "ggi_R",     0.0,  0.04, 0.8, weld=("C","L"), cam="close_l"),
    S("Lift Green",        "lg_L",      "lgi_R",     0.0,  0.04, 1.0, weld=("C","L"), cam="side"),
    S("Place Green",       "pg_L",      "pgi_R",     0.0,  0.04, 1.0, weld=("C","L"), cam="overhead"),
    S("Release Green",     "relg_L",    "relgi_R",   0.04, 0.04, 0.6, cam="overhead"),
    # Phase 3: 红块叠在绿上（含故障恢复）
    S("Reach Red",         "rr2_L",     "rr2i_R",    0.04, 0.04, 1.0, cam="close_l"),
    S("Grasp Red",         "gr2_L",     "gr2i_R",    0.0,  0.04, 0.8, weld=("B","L"), cam="close_l"),
    S("Lift Red",          "lr2_L",     "lr2i_R",    0.0,  0.04, 1.0, weld=("B","L"), cam="side"),
    S("Stack Approach",    "stk_L",     "stki_R",    0.0,  0.04, 1.0, weld=("B","L"), cam="overhead"),
    # ★ 故障恢复场景
    S("Stack Misalign",    "fail_L",    "fail_R",    0.0,  0.04, 0.8, weld=("B","L"), cam="close_l"),
    S("Recover Stack",     "fix_L",     "fix_R",     0.0,  0.04, 0.8, weld=("B","L"), cam="close_l"),
    S("Stack Final",       "stk2_L",    "stki2_R",   0.0,  0.04, 0.8, weld=("B","L"), cam="overhead"),
    S("Release Red",       "fin_L",     "fin_R",     0.04, 0.04, 0.6, cam="overhead"),
    S("Complete",          "fin_L",     "fin_R",     0.04, 0.04, 1.0, cam="wide"),
]
...
CAM_D = {"wide":(30,-25,5.0,0.1), "close_l":(20,-35,3.2,0.05), "close_r":(40,-35,3.2,0.05),
         ...
         "ultra_close":(15,-40,2.5,0.03), "dramatic":(45,-15,6.0,0.15)}
...
def overlay(si, t, fL, fR, gL, gR, fh, err=False, phase=""):
    ...
    for i in range(st,en):
        ...
        if i==si:
            ...
            draw.text((30,y), f"▶ {LABELS[i]}", fill=C_G if not err else C_R, font=FONT_XS)
    ...
    draw.text((px+10,py+8), "FORCE FEEDBACK", fill=C_A, font=FONT_M)
...
print("\nRendering v18...")
mujoco.mj_resetData(model, data); mujoco.mj_forward(model, data)
frames=[]; fh=[]; prev_cp="overhead"
cL=P["scan_L"][:]; cR=P["scan_R"][:]; cfL=0.04; cfR=0.04
t0=time.time(); fc=0
...
for si,(name,tL,tR,fl,fr,dur,weld_info,cp) in enumerate(STEPS):
    ...
    is_err=("Fail" in name or "Misalign" in name or "Re-align" in name)
